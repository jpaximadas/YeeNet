...
bool host_link_tx_in_progress = false;
...
void host_link_modem_listen(uint8_t *command, uint16_t len){
    bool status = modem_listen(&host_link_modem);
    link.iface_write_bytes((uint8_t *)&status,1);
}

void host_link_modem_load_payload(uint8_t *command, uint16_t len){
    if(host_link_tx_in_progress || !modem_is_clear(&host_link_modem)){
        uint8_t busy = 0x02;
        link.iface_write_bytes(&busy,1);
    }
    ...
    link.iface_write_bytes((uint8_t *)&fits,1);
    return;
}
...
//requires a byte of length in command
