 ...
 * It performs a few important tasks that makes using a modem more reliable and eliminates timing considerations from the design of higher layers:
 *  - Prevents transmission during reception of a packet
 *  - Detects whether a packet was successfully (method for this depends on whether the packet was ACKED/UNACKED)
 *  - Responds to detected failures of packet transmission in PERSISTENT modem in a way that resolves collisions (exponential backoff)
 ...
 *  - Completes tranmissions of a packet in a finite amount of time while providing indication whether packet tranmission was successful
 *  
 * 
 */
...
enum send_mode{
	 ...
	 * 
	 * PACKET_UNACKED: Reception of a NACK from the destination results in FAILED packet handler_state. Absence of a NACK results in SUCCESS packet handler state
	 * 
	 * PACKET_ACKED: NACKS are ignored since they are sometimes emitted when LoRa modems parse noise (mostly in SF6). ACK from destination node results in SUCCESS packet handler state.
	 * Absence of am ack results in FAILED packet handler state.
	 */
	LAZY=0, 
	/**
	 * Packet handler will attempt to retransmit until reception success or number of retransmissions equals packet_handler member "backoffs_max"
	 * 
	 * PACKET_UNACKED: Reception of a NACK from the destination results in retransmission of packet or failure when backoffs_max is reached. Absence of a NACK results in SUCCESS packet handler state
	 * 
	 * PACKET_ACKED: NACKS are ignored since they are sometimes emitted when LoRa modems parse noise (mostly in SF6). ACK from destination node results in SUCCESS packet handler state.
	 * Absence of an ack results in successive, delayed retransmissions (exponential backoff).
	 */
	PERSISTENT
};
...
struct packet_handler{
	...
	/** defines how the packet handler deals with reception failures in destination nodes. Enum definition contains more details. */
	...
	/** <b>internal variable</b> indicates how many times a retransmission has been attempted in PERSISTENT mode*/
	uint8_t backoffs;

	/** <b>internal variable</b> max number of times a retransmission can occur in PERSISTENT mode */
	uint8_t backoffs_max;

	/** <b>internal variable</b> indicates whether a retransmission attempted to start while the modem was busy receiving a packet */
	bool tx_snooze;
 ...
 * @param _pkt_rdy_callback function pointer to be dereferenced after an incoming packet is written to _rx_packet
 ...
 * 
 * If both checks pass, the packet handler is reset for a new packet, the packet is transmitted, and true is returned
 * If either check fails, false is returned.
 * packet_handler->my_state can be polled to avoid performing a function call however even if the handler state is UNLOCKED this does not preclude the possibility that the modem will be busy.
 * 
